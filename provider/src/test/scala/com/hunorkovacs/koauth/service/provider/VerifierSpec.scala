...
class VerifierSpec(implicit ee: ExecutionEnv) extends Specification with Mockito with Scope {
  ...
  val SignatureBase: String = "POST&https%3A%2F%2Fapi.twitter.com%2F1%2Fstatuses%2Fupdate.json" +
  ...
  val BaseWithCallback: String = "POST&https%3A%2F%2Fapi.twitter.com%2F1%2Fstatuses%2Fupdate.json" +
    "&include_entities%3Dtrue" +
  ...
  "Verifying signature" should {
    "return positive verification if signature matches." in new commonMocks {
      val request: KoauthRequest = KoauthRequest(Method, Url, UrlParams, BodyParams, ParamsList)
    ...
    "return negative verification if signature doesn't match." in new commonMocks {
      val paramsList: List[(String, String)] = ParamsList.filterNot(e => "oauth_signature".equals(e._1))
  ...
  "Verifying signature method" should {
    ...
    "return unsupported verification if method is other than HMAC-SHA1." in new commonMocks {
      val paramsList: List[(String, String)] = ParamsList.filterNot(e => "oauth_signature_method".equals(e._1))
  ...
  "Verifying timestamp" should {
    "return positive verification if timestamp equals current time." in new commonMocks {
      val paramsList: List[(String, String)] = ParamsList.filterNot(e => "oauth_timestamp".equals(e._1))
    ...
    "return positive verification if timestamp is 9 minutes late." in new commonMocks {
      val nineMinutesAgo: Long = now - 9 * 60
      val paramsList: List[(String, String)] = ParamsList.filterNot(e => "oauth_timestamp".equals(e._1))
    ...
    "return negative verification if timestamp is 11 minutes late." in new commonMocks {
      val nineMinutesAgo: Long = now - 11 * 60
      val paramsList: List[(String, String)] = ParamsList.filterNot(e => "oauth_timestamp".equals(e._1))
        .::(("oauth_timestamp", nineMinutesAgo.toString))
      val request: KoauthRequest = KoauthRequest(Method, Url, UrlParams, BodyParams, paramsList)
  ...
  "Verifying required parameters" should {
    ...
    "return positive if lists are the same but shuffled." in new commonMocks {
      val request: KoauthRequest = createRequest(List(("b", "2"), ("a", "1")))
    ...
    "return negative if parameter is missing and state which." in new commonMocks {
      val request: KoauthRequest = createRequest(List(("a", "1")))
    ...
    "return negative if duplicate parameter and state which." in new commonMocks {
      val request: KoauthRequest = createRequest(List(("a", "1"), ("b", "2"), ("b", "3")))
      verifier.verifyRequiredParams(request, List("a", "b")) must
        beEqualTo (VerificationUnsupported(MessageParameterMissing + "b"))
    }
    "return positive if additional parameter and state which." in new commonMocks {
      val request: KoauthRequest = createRequest(List(("a", "1"), ("b", "2"), ("c", "3")))
      verifier.verifyRequiredParams(request, List("a", "b")) must beEqualTo (VerificationOk)
    }
  }
  ...
  "Verifying the four verifications" should {
    "return positive if signature, method, timestamp, nonce all ok." in new commonMocks {
      ...
      private val signatureBase = actualizeSignatureBase(SignatureBase, time)
      private val signature = sign(signatureBase, ConsumerSecret, TokenSecret)
      private val paramsList = actualizeParamsList(ParamsList, signature, time)
      private val request = KoauthRequest(Method, Url, UrlParams, BodyParams, paramsList)
      mockedPer.nonceExists(Nonce, ConsumerKey, Token) returns successful(false)
    ...
    "return negative if method, timestamp, nonce all ok but signature is invalid." in new commonMocks {
      private val time = now
      private val signatureBase = actualizeSignatureBase(SignatureBase, time)
      private val paramsList = actualizeParamsList(ParamsList, "#:|^*&invalidsignature", time)
      private val request = KoauthRequest(Method, Url, UrlParams, BodyParams, paramsList)
      mockedPer.nonceExists(Nonce, ConsumerKey, Token) returns successful(false)
  ...
  "Verifying the 'Request Token' request" should {
    "return positive Consumer Key exists." in new commonMocks {
      private val time = now
      private val signatureBase = actualizeSignatureBase(BaseWithCallback, time)
        .replaceFirst("%26oauth_token%3D370773112-GmHxMAgYyLbNEtIKZeRNFsMKPR9EyMZeS9weJAEb", "")
      private val signature = sign(signatureBase, ConsumerSecret, "")
      private val paramsList = actualizeParamsList(ParamsListWithCallback, signature, time)
      private val request = KoauthRequest(Method, Url, UrlParams, BodyParams, paramsList)
    ...
    "return negative if Consumer Key is not registered." in new commonMocks {
      ...
      private val request = KoauthRequest(Method, Url, UrlParams, BodyParams, ParamsListWithCallback)
  ...
  "Verifying the requests with Token" should {
    ...
    "return negative if token with consumer key doesn't exist." in new commonMocks {
      mockedPer.getConsumerSecret(ConsumerKey) returns successful(Some(ConsumerSecret))
      private def cantGetSecret(consumerKey: String, token: String) = successful(None)
      private val request = KoauthRequest(Method, Url, UrlParams, BodyParams, ParamsList)
    ...
    "return negative if required parameter is missing." in new commonMocks {
      val params: List[(String, String)] = ParamsListWithCallback.filterNot(p => p._1 == "oauth_version")
      val request: KoauthRequest = KoauthRequest(Method, Url, UrlParams, BodyParams, params)

      val verification: Verification = Await.result(verifier.verifyWithToken(request, OauthenticateRequiredParams, getSecret), 1.0 second)

      val message: Object = verification match {
  ...
  private trait commonMocks extends Before with Mockito with ThrownExpectations {
    implicit lazy val mockedPer: Persistence = mock[Persistence]
    val verifier = new CustomVerifier(mockedPer, ec)

    override def before: Nil.type = Nil
  }
}
