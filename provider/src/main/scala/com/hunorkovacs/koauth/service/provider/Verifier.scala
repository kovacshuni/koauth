...
protected class CustomVerifier(private val persistence: Persistence, private val ec: ExecutionContext) extends Verifier {
            ...
            case Some(someConsumerSecret) =>
              Future(request.oauthParamsMap(TokenName)) flatMap { token =>
                getSecret(consumerKey, token) flatMap {
                  case None =>
                    logger.debug("Token {} with Consumer Key {} does not exist. Request id: {}", token, consumerKey, request.id)
  ...
  def fourVerifications(request: KoauthRequest, consumerSecret: String, token: String, tokenSecret: String): Future[Verification] = {
    verifyNonce(request, token) flatMap { nonceVerification =>
      Future {
        List(verifySignature(request, consumerSecret, tokenSecret), verifyAlgorithm(request), verifyTimestamp(request))
          .::(nonceVerification)
  ...
  def verifySignature(request: KoauthRequest, consumerSecret: String, tokenSecret: String): Verification = {
    ...
    if (sentSignature.equals(computedSignature)) VerificationOk
      ...
      logger.debug(
        "Signature does not match. Given: {}; Computed: {}; Computed signature base: {}; Request id: {}",
        sentSignature,
        computedSignature,
        signatureBase,
        request.id
  ...
  def verifyTimestamp(request: KoauthRequest): Verification = {
    ...
    try {
      ...
      case nfEx: NumberFormatException =>
        logger.debug("Invalid timestamp format. Given timestamp: {}; Expecting a stamp close to this: {};", timestamp, expectedStamp)
  ...
  def verifyAlgorithm(request: KoauthRequest): Verification = {
    ...
    if (HmacReadable != signatureMethod) {
      logger.debug("Unsupported Signature Method. Given: {}; Required: {}. Request id: {}", signatureMethod, HmacReadable, request.id)
  ...
  def verifyRequiredParams(request: KoauthRequest, requiredParams: List[String]): Verification = {
    ...
    if (diff.isEmpty && paramsKeys.distinct.size == paramsKeys.size) VerificationOk
      ...
      logger.debug(
        "Given OAuth parameters are not as required. Given: {}; Required: {}; Request id: {}",
        paramsKeys,
        requiredParams,
        request.id
      )
...
protected object VerifierObject {
  ...
  val TimePrecisionSeconds: Int = 10 * 60

  final val RequestTokenRequiredParams =
    List[String](ConsumerKeyName, SignatureMethodName, SignatureName, TimestampName, NonceName, VersionName, CallbackName).sorted
  final val AccessTokenRequiredParams =
    List[String](ConsumerKeyName, TokenName, SignatureMethodName, SignatureName, TimestampName, NonceName, VersionName, VerifierName).sorted
  final val OauthenticateRequiredParams =
    List[String](ConsumerKeyName, TokenName, SignatureMethodName, SignatureName, TimestampName, NonceName, VersionName).sorted
