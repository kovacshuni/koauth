...
import com.hunorkovacs.koauth.domain.{KoauthRequest, KoauthResponse, ResponseBadRequest, ResponseNok, ResponseUnauthorized}
...
import com.hunorkovacs.koauth.service.provider.VerifierObject.{MessageNotAuthorized, MessageUserInexistent}
...
protected class CustomProviderService(
    private val oauthVerifier: Verifier,
    ...
    verifyForOauthenticate(request) flatMap {
      ...
      case VerificationOk =>
        ...
        persistence.getUsername(consumerKey, token) flatMap {
          case None =>
            logger.debug("User does not exist for Consumer Key {} and Access Token {}. Request id: {}", consumerKey, token, request.id)
