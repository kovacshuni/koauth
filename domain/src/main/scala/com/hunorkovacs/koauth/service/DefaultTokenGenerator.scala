...
object DefaultTokenGenerator extends TokenGenerator {
  ...
  override def generateTokenAndSecret: (String, String) = (generate(LengthToken), generate(LengthSecret))

  override def generateVerifier: String = generate(LengthVerifier)

  override def generateNonce: String = generate(LengthNonce)
