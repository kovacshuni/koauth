...
import com.hunorkovacs.koauth.domain.{KoauthRequest, ResponseOk, TokenResponse}
...
class ArithmeticsSpec extends Specification {
  ...
  val NormalizedRequestParams: String = "include_entities=true&" +
    "oauth_consumer_key=xvz1evFS4wEEPTGEFPHBog&" +
    "oauth_nonce=kYjzVBB8Y0ZFabxSWbWovY3uYSQ2pTgmZeNu2VS4cg&" +
    "oauth_signature_method=HMAC-SHA1&" +
    "oauth_timestamp=1318622958&" +
  ...
  "Encoding, pairing, sorting and concatenating" should {
    "encode, pair keys with values by equals sign and concatenate params with ampersand." in {
      encodePairSortConcat(List(("oauth_token", "ab3cd9j4ks73hf7g"), ("oauth_token_secret", "xyz4992k83j47x0b"))) must
        equalTo(
    ...
    "percent encode key and values." in {
      encodePairSortConcat(
        List(("oauth_token", "hi there, this is a colon at the end:"), ("oauth_token_secret", "oh thank you / you're welcome"))
      ) must
        equalTo(
          "oauth_token=hi%20there%2C%20this%20is%20a%20colon%20at%20the%20end%3A&" +
  ...
  "Concatenating Request Elements For Signature" should {
    ...
    "have a double encoded callback URL at the end." in {
      ...
      concatItemsForSignature(request) must equalTo(
        "POST&" +
          "https%3A%2F%2Fapi.twitter.com%2F1%2Fstatuses%2Fupdate.json&" +
          "include_entities%3Dtrue%26" +
    ...
    "use lowercase URL." in {
      val request = KoauthRequest(Method, "HTTpS://Api.Twitter.com/1/Statuses/Update.JSON", UrlParams, BodyParams, OauthParamsList)
      concatItemsForSignature(request) must
        equalTo(
          SignatureBase.replaceFirst(
            "https%3A%2F%2Fapi.twitter.com%2F1%2Fstatuses%2Fupdate.json",
            "https%3A%2F%2Fapi.twitter.com%2F1%2FStatuses%2FUpdate.JSON"
          )
        )
    ...
    "include specific port." in {
      val request = KoauthRequest(Method, "https://api.twitter.com:9000/1/statuses/update.json", UrlParams, BodyParams, OauthParamsList)
      concatItemsForSignature(request) must
