...
import com.hunorkovacs.koauth.service.DefaultConsumerService.{MessageMissingConsumerSecret, signRequest, createSignatureBase, createGeneralSignedRequest}
...
class ConsumerServiceSpec extends Specification {
  ...
  "Creating a general signed request" should {
    "sign correctly and include signature in Authorization header together with the rest of the parameters." in {
      ...
      createGeneralSignedRequest(request) must beEqualTo(Right(AuthHeader)).await
    }
    "is not possible without Consumer Secret." in {
      val params = OauthParamsList.filterNot(p => consumerSecretName == p._1)
      val request = new EnhancedRequest(Method, Url, UrlParams, BodyParams, params, params.toMap)

      createGeneralSignedRequest(request) must beEqualTo(Left(MessageMissingConsumerSecret)).await
    }
  }
  ...
  "Signing a request " should {
    "give the correct signature." in {
      ...
      signRequest(request) must beEqualTo(Right(Signature)).await
    }
    "give the correct signature when no Token Secret is present." in {
      val params = OauthParamsList.filterNot(p => tokenSecretName == p._1)
      val request = new EnhancedRequest(Method, Url, UrlParams, BodyParams, params, params.toMap)

      signRequest(request) must beEqualTo(Right("KaRibr4jurQUGNDvM5Kp+qd4AHw=")).await
    }
    "not be possible without Consumer Secret." in {
      val params = OauthParamsList.filterNot(p => consumerSecretName == p._1)
      val request = new EnhancedRequest(Method, Url, UrlParams, BodyParams, params, params.toMap)

      signRequest(request) must beEqualTo(Left(MessageMissingConsumerSecret)).await
    }
  }
